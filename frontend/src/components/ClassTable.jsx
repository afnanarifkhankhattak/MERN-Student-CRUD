// frontend/src/components/ClassTable.jsx

import { useEffect, useState } from 'react';
import { CLASSES_URL as API_URL, apiFetch } from '../api';

function ClassTable({ refreshTrigger, onEdit, onDelete }) {
  ...
  if (loading) return <p style={styles.info}>Loading classes...</p>;
  if (error) return <p style={styles.error}>Error: {error}</p>;
  if (classes.length === 0)
    return <p style={styles.info}>No classes yet. Add one above! 👆</p>;
    ...
    <div style={styles.container}>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={styles.table}>
          ...
          <tbody>
            {classes.map((cls, i) => (
              <tr key={cls._id} style={styles.bodyRow}>
                <td style={styles.td}>{i + 1}</td>
                <td style={styles.td}>
                  <strong>{cls.name}</strong>
                </td>
                <td style={styles.td}>{cls.numericLevel}</td>
                <td style={styles.td}>
                  {cls.isActive ? (
                ...
                <td style={styles.td}>{cls.description || '—'}</td>
                <td style={styles.td}>
